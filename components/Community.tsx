...
import { Heart, MessageSquare, Send, User, Hash, MoreHorizontal, Share2, Check, Image as ImageIcon, Trash2, Shield, WifiOff, Feather } from 'lucide-react';
...
const Community: React.FC = () => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Basic size check (Firestore document limit is 1MB, so keep images small)
      if (file.size > 800 * 1024) {
        alert("이미지 크기는 800KB 이하여야 합니다.");
        return;
      }
  ...
  const handleCreatePost = async (e: React.FormEvent) => {
    e.preventDefault();
    if ((!newPostContent.trim() && !newPostImage) || !userProfile || isSubmitting) return;
              ...
              // LOGGED IN: Show Profile & Write Form
               <div className="lg:sticky lg:top-32 bg-[#111] border border-stone-800 p-6 shadow-2xl relative overflow-hidden group rounded-sm">
                  ...
                  <form onSubmit={handleCreatePost} className="space-y-4">
                     ...
                     {/* Image Preview */}
                     {newPostImage && (
                       <div className="relative w-full h-32 bg-black/50 rounded overflow-hidden border border-stone-800">
                         <img src={newPostImage} alt="Preview" className="w-full h-full object-cover opacity-80" />
                         <button 
                           type="button"
                           onClick={() => setNewPostImage(null)}
                           className="absolute top-2 right-2 bg-black/70 text-white p-1 rounded-full hover:bg-red-900 transition-colors"
                         >
                           <Trash2 size={14} />
                         </button>
                       </div>
                     ...
                     <div className="flex items-center justify-between pt-2">
                        ...
                        <button 
                          ...
                          className="text-stone-500 hover:text-gold-500 transition-colors p-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                           <ImageIcon size={20} />
                        </button>
             ...
             {posts.map((post) => (
               <div key={post.id} className="bg-[#111] border border-stone-800/60 p-6 md:p-8 hover:border-stone-700 transition-all duration-300 group rounded-sm shadow-xl">
                  ...
                  <div className="mb-6 pl-13">
                     ...
                     {/* Image Attachment */}
                     {post.image && (
                       <div className="mt-4 rounded border border-stone-800 overflow-hidden">
                         <img src={post.image} alt="Attached content" className="w-full max-h-[400px] object-cover hover:opacity-90 transition-opacity" />
                       </div>
                     )}
